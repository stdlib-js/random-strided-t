...
/**
* Interface for filling strided arrays with pseudorandom numbers drawn from a Student's t distribution.
*/
interface Random extends PRNG {
	...
	*/
	<T = unknown>( N: number, v: Collection<number>, sv: number, out: Collection<T>, so: number ): Collection<T | number>;

	/**
	* Fills a strided array with pseudorandom numbers drawn from a Student's t distribution using alternative indexing semantics.
	*
	* @param N - number of indexed elements
	* @param v - degrees of freedom
	* @param sv - `v` strided length
	...
	*/
	ndarray<T = unknown>( N: number, v: Collection<number>, sv: number, ov: number, out: Collection<T>, so: number, oo: number ): Collection<T | number>;
}
...
interface Routine extends Random {
	/**
	* Returns a function for filling strided arrays with pseudorandom numbers drawn from a Student's t distribution.
	*
